...
- **차분하고 미니멀한 한국어 에디토리얼 톤.** 각 카드는 고유한 비주얼을 가집니다 — 소스에 실제 영상이 있으면 짧은 무음 비디오 클립을, 없으면 기사 본문에서 가져온 이미지를 씁니다.
...
cards.json ────────▶ 5 ax-media     ─▶ media.json + 영상/이미지
cards + media ─────▶ 6 ax-publisher ─▶ axbrief-data.js + 아카이브
```

| 에이전트 | 역할 | 핵심 규칙 |
| --- | --- | --- |
| `ax-planner` | 오늘의 검색 키워드 기획. | `core`(`AI Design Tool` · `Design AX` · `AI Workflow`)는 항상 사용; 광범위한 `keyword_pool.json`에서 날짜 기준으로 부분집합을 로테이션; 총 ~20–28개 쿼리; 최근 다룬 주제는 회피. |
...
| `ax-media` | 카드별 고유 비주얼 부여. | 소스에 실제 영상이 있으면 **짧은 무음 비디오 클립**(H.264 MP4 ≈8초, 포스터 still 동반)을 1순위 → **기사 본문 내부 이미지**(제목과 본문 사이) → og:image → 생성 SVG. md5 비교로 두 카드가 같은 비주얼을 쓰지 않게 함(영상은 절대 조작하지 않음 — 실제 소스 영상이 없으면 이미지로). |
...
> **키워드 풀 & 로테이션.** 모든 검색 키워드는 `pipeline/keyword_pool.json`에서 관리됩니다. 매일 항상 검색하는 `core`는 **`AI Design Tool` · `Design AX` · `AI Workflow`** 세 개이며, 더 큰 `pool`은 7개 카테고리로 분류됩니다:
>
> | 카테고리 | 내용 | 예시 |
> | --- | --- | --- |
> | `tools_creative` | 생성형 미디어 툴 | Higgsfield, Kling, Runway, Sora, Midjourney, Veo, Luma, Pika, Flux |
> | `tools_design_code` | 디자인·코드 툴 | Figma, Claude Code, v0, Framer AI, Cursor, Lovable, Bolt, Vibe coding, MCP |
> | `models_labs` | 모델 & 랩 | OpenAI, ChatGPT, Anthropic, Gemini, Meta, Llama, Mistral, Grok, DeepSeek |
> | `concepts` | 핵심 개념 | AI Agent, Agentic AI, Physical AI, World models, Multimodal AI |
> | `people` | 인물 | LeCun, Karpathy, Amodei, Altman, Hassabis, Fei-Fei Li, Sutskever, Jensen Huang |
> | `frontier_broad` | 프런티어·하드웨어 | Nvidia, Figure AI, Optimus, Boston Dynamics, Vision Pro, Suno, AGI, AI 규제 |
> | `social` | 소셜·튜토리얼 | Figma/Vercel YouTube, Matt Wolfe, Fireship, Awwwards, design AI reel |
>
> 플래너는 작은 `core`는 매일 검색하고, `pool`에서는 매일 다른 부분집합을 날짜 기준으로 로테이션합니다(+ SKILL Step 0에서 사용자가 입력한 키워드를 그 위에 더함) — 이로써 커버리지는 넓히면서 하루 검색량은 적정 수준으로 유지합니다.
...
- **히어로 캐러셀** — 오늘의 최대 5장. 스와이프 가능하며 각 카드에 영상 또는 이미지·헤드라인·본문·원문 링크.
...
- **풀블리드 카드 5장** — 오늘의 소식을 화면을 꽉 채운 배너 카드로; 각 카드 배경은 기사 원문 영상(자동 재생·무음·루프) 또는 이미지.
...
```
index.html                        # 루트(/) — small/캐러셀 디자인 (배포 진입점)
axbrief-app.jsx                   # small 디자인 React UI (히어로 캐러셀·주간 덱)
large.html                        # /large — large card 디자인
axbrief-app-large.jsx             # large 디자인 React UI (스크롤 확장 히어로·풀블리드 카드·틸트 아카이브)
"Design AX Brief.html" / "(Large Card).html"  # 동일 페이지의 옛 경로(하위호환용으로 유지)
...
pipeline/
  ...
  media/                # 다운로드된 카드 비주얼 — 영상(.mp4 + 포스터 .jpg)/이미지(추적); deck/ = 주간 배너
...
# 접속: http://localhost:8765/  (small/캐러셀) · http://localhost:8765/large.html (large card)
```
...
### 배포 (선택)
정적 사이트입니다. 두 페이지(`index.html` → small, `large.html` → large)와 두 앱(`axbrief-app.jsx`, `axbrief-app-large.jsx`), 공유 데이터 `axbrief-data.js`, `pipeline/media/**`(영상·이미지 파일은 추적됨)를 커밋한 뒤 Cloudflare Workers/Pages · GitHub Pages · Netlify 등 임의의 정적 호스트에 올리면 `/` 와 `/large` 로 깔끔하게 서빙됩니다. 두 페이지가 같은 데이터를 읽으므로 함께 배포하면 동시 발행됩니다. 중간 파이프라인 JSON과 `pipeline/runs/` 는 git에서 제외됩니다.
