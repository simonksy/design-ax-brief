...
```
Design AX Brief.html        # 페이지 (이 파일을 열면 됨)
axbrief-app.jsx             # React UI (히어로·덱·모티프) — 브라우저에서 Babel 컴파일
axbrief-data.js             # 자동 생성 데이터 (window.AX_NEWS + window.AX_DAYS)
.claude/
...
### 배포 (선택)
정적 사이트입니다. `Design AX Brief.html`, `axbrief-app.jsx`, `axbrief-data.js`, `pipeline/media/**`(이미지 파일은 추적됨)를 커밋한 뒤 GitHub Pages / Netlify / 임의의 정적 호스트에 올리면 됩니다. 중간 파이프라인 JSON과 `pipeline/runs/` 는 git에서 제외됩니다.
